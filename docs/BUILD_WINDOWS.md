...
toolchain (CMake 4.3.2, Boost 1.83, MariaDB 10.6.3+).
...
> Current verification note: the Linux `bnetserver` target has been verified with
> Boost 1.83. A full Windows `worldserver`/scripts build has not yet been run in
> this recovery session, but the documented toolchain and paths match the current
> CMake files.
...
| Boost | **1.83.0** | prebuilt binaries: <https://sourceforge.net/projects/boost/files/boost-binaries/1.83.0/> → pick `boost_1_83_0-msvc-14.3-64.exe` |
...
If you install Boost manually instead of using `compile_deps`, set the environment variable so CMake finds it:

```powershell
[Environment]::SetEnvironmentVariable('BOOST_ROOT', 'C:\local\boost_1_83_0', 'User')
```
...
* Uses `compile_deps/cmake/LegionCoreDeps.cmake` to find dependencies under `compile_deps/` after `setup_deps.ps1`; user-supplied `-D..._ROOT=...` overrides still win
...
-- Found Boost: ... (found suitable version "1.83.0", minimum required is "1.83")
-- Configuring done
-- Generating done
```
...
2. **`The Boost C++ Libraries were not found`** — the Boost prebuilt installer
   ...
   `boost_1_83_0-msvc-14.3-64.exe` — the `-14.3` part **must** match VS 2022.
   VS 2019 is not supported by this branch; use VS 2022 / MSVC v143.
...
For standalone/portable server layout after compiling, see [`PORTABLE_SERVER_SETUP.md`](PORTABLE_SERVER_SETUP.md).
